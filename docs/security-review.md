...
- `COVEO_PLATFORM_API_KEY` is server-side only for RGA, Technical Resources, the conversational agent (`/api/coveo/conversation`), and the server-rendered product detail lookup (`fetchProductDetail`, `src/lib/coveo/product-detail.ts`).
...
- The server-rendered product detail page looks up a single product by exact `@permanentid`/`@ec_product_id` (`fetchProductDetail`, `src/lib/coveo/product-detail.ts`) using the server-only `COVEO_PLATFORM_API_KEY`. The client-controlled route id is escaped before it is interpolated into the Search API query expression, only one result is requested, and the function returns `undefined` (never throws) on a miss or upstream failure so the page degrades to its empty state — a narrow single-document lookup, not a general search proxy.
- The product detail page's sessionStorage handoff (`src/lib/commerce/product-session-cache.ts`) now serves as a fallback when the server lookup misses; it carries the same `ProductResult` the browser already fetched from Coveo for the active session, same-origin only; it is not user-controlled input and introduces no new trust boundary.
- `.env.example` contains placeholders only.
- Every response carries a `Content-Security-Policy` (default-src `'self'`, `connect-src` additionally scoped to `https://*.cloud.coveo.com https://*.org.coveo.com` for the browser-side Headless search-token flow), `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, `Referrer-Policy: strict-origin-when-cross-origin`, and a `Permissions-Policy` denying geolocation/camera/microphone (`next.config.mjs`). `script-src` includes `'unsafe-inline'` because Next.js injects its own inline hydration/RSC-payload scripts on every page (verified via production build + Playwright: `script-src 'self'` alone blocks the app from hydrating at all); a stricter nonce-based policy would need per-request middleware, tracked as future work rather than attempted in this pass. In non-production (`NODE_ENV !== "production"`) only, `script-src` additionally includes `'unsafe-eval'`, required because Next.js dev mode's webpack bundler evaluates chunks via `eval()` for fast-refresh source maps; the production policy served by `npm run build`/`npm start` never includes `'unsafe-eval'`.
