...
- `/api/search-token`, `/api/coveo/generative/answer`, and `/api/coveo/conversation` — the three privileged, Coveo-calling routes — are each rate-limited per client IP (`src/lib/http/rate-limit.ts`, in-memory sliding window; noted there as needing a shared store for a multi-instance deployment) and time out their outbound Coveo fetches via `AbortSignal.timeout(...)` rather than hanging indefinitely.
